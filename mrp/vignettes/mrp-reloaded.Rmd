...
```{r loading-the-package, message=FALSE}
library(mrp)
```
...
> Congress considered many important bills over the past two years. For each of the following tell us whether you support or oppose the legislation in principle: Constitutional Amendment banning Gay Marriage ("Support" coded 1; "Oppose" 0).

```{r mrp-simple, message=FALSE, results='hide'}
mrp.simple <- mrp(ban.gaymarr ~ state+age+education, 
                  data=CCES.complete,
                  population=mrp.census,
                  pop.weights="weighted2008")
```
### What's happening inside the `mrp()` black box? 

1. Stratify the poll data according to the *formula.cell*. This creates an N-way array of data where the dimensions are given by the categories of the stratification variables. On this array, it computes the mean of the binary responses in each cell, and the effective N taking into account any survey weights provided, and the “design effect” in that cell of the array. 

1. Stratify the census data according to the same formula. The dimensions are checked and if the census contains an extra level on any dimension (states with no survey respondents, for example), those cells are created in the survey N-way data as well, with $\bar{Y}=0.5$ and cell effective N of 0. 

1. Collapse the aligned N-way structure back to a two-dimensional matrix with factors for array extent indexing. The model-fitting functions will turn these into indicator matrices transparently.

1. With the model data, do any transformations specified in *grouplevel.expressions* and `join` the members of *grouplevel.data.frames*.

1. Fit the multilevel model. By default *formula.model* just fits varying intercepts for the same terms as the *formula.cell*.
...
```{r poststratify-simple,results='asis',message=FALSE}
library(xtable)
table <- poststratify(mrp.simple, ~ education + age)
print(xtable(table,
...
```{r map-simple,dev='quartz_png', fig.height=7.5, fig.width=9}
spplot(mrp.simple, state~age+education, exclude=c("AK","HI"))
```